...
class CMB2_Ajax {
	...
	public function oembed_handler() {
		...
		if ( empty( $oembed_string ) ) {
			wp_send_json_error( '<p class="ui-state-error-text">' . esc_html__( 'Please Try Again', 'meridian-recipes' ) . '</p>' );
		}
	...
	public function get_oembed( $args ) {
		...
		if ( $check_embed && $check_embed != $fallback ) {
			return '<div class="embed-status">' . $check_embed . '<p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button" rel="' . $args['field_id'] . '">' .esc_html__( 'Remove Embed', 'meridian-recipes' ) . '</a></p></div>';
		}

		// Otherwise, send back error info that no oEmbeds were found
		return '<p class="ui-state-error-text">' . sprintf(esc_html__( 'No oEmbed Results Found for %s. View more info at', 'meridian-recipes' ), $fallback ) . ' <a href="http://codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>.</p>';
