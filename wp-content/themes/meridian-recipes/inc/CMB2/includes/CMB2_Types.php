...
class CMB2_Types {
	...
	public function render_repeatable_field() {
		...
		<p class="cmb-add-row">
			<button data-selector="<?php echo esc_attr( $table_id ); ?>" class="cmb-add-row-button button"><?php echo esc_html( $this->_text( 'add_row_text', esc_html__( 'Add Row', 'meridian-recipes' ) ) ); ?></button>
		</p>
	...
	protected function repeat_row( $disable_remover = false, $class = 'cmb-repeat-row' ) {
		...
		<div class="cmb-row <?php echo esc_attr( $class ); ?>">
			...
			<div class="cmb-td cmb-remove-row">
				<button class="button cmb-remove-row-button<?php echo esc_attr( $disabled ); ?>"><?php echo esc_html( $this->_text( 'remove_row_text', esc_html__( 'Remove', 'meridian-recipes' ) ) ); ?></button>
			</div>
		</div>
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= sprintf( '<li><label>%s</label></li>', esc_html( $this->_text( 'no_terms_text', esc_html__( 'No terms', 'meridian-recipes' ) ) ) );
		} else {
			$option_none  = $this->field->args( 'show_option_none' );
			if ( ! empty( $option_none ) ) {
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => esc_html( $this->_text( 'add_upload_files_text', esc_html__( 'Add or Upload Files', 'meridian-recipes' ) ) ),
			'name'  => '', 'id'  => '',
		) );
	...
	public function file() {
		...
		printf( '<input class="cmb2-upload-button button" type="button" value="%s" />', esc_attr( $this->_text( 'add_upload_file_text', esc_html__( 'Add or Upload File', 'meridian-recipes' ) ) ) );
	...
	public function img_status_output( $args ) {
		printf( '<%1$s class="img-status">%2$s<p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button"%3$s>%4$s</a></p>%5$s</%1$s>',
			...
			esc_html( $this->_text( 'remove_image_text', esc_html__( 'Remove Image', 'meridian-recipes' ) ) ),
	...
	public function file_status_output( $args ) {
		printf( '<%1$s class="file-status"><span>%2$s <strong>%3$s</strong></span>&nbsp;&nbsp; (<a href="%4$s" target="_blank" rel="external">%5$s</a> / <a href="#" class="cmb2-remove-file-button"%6$s>%7$s</a>)%8$s</%1$s>',
			$args['tag'],
			esc_html( $this->_text( 'file_text', esc_html__( 'File:', 'meridian-recipes' ) ) ),
			$this->get_file_name_from_path( $args['value'] ),
			$args['value'],
			esc_html( $this->_text( 'file_download_text', esc_html__( 'Download', 'meridian-recipes' ) ) ),
			isset( $args['cached_id'] ) ? ' rel="' . $args['cached_id'] . '"' : '',
			esc_html( $this->_text( 'remove_text', esc_html__( 'Remove', 'meridian-recipes' ) ) ),
			isset( $args['id_input'] ) ? $args['id_input'] : ''
		);
	}
