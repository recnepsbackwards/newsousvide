...
class CMB2_Field {
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => esc_html__( 'Add Group', 'meridian-recipes' ),
			'remove_button' => esc_html__( 'Remove Group', 'meridian-recipes' ),
		) ) : $args['options'];
		...
		if ( in_array( $args['type'], $option_types, true ) ) {
			...
			$args['show_option_none'] = true === $args['show_option_none'] ? esc_html__( 'None', 'meridian-recipes' ) : $args['show_option_none'];

			if ( ! $args['show_option_none'] ) {
				$off_by_default = in_array( $args['type'], array( 'select', 'radio', 'radio_inline' ), true );
				$args['show_option_none'] = $off_by_default ? false : esc_html__( 'None', 'meridian-recipes' );
			}
